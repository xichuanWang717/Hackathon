...
(function () {
  ...
  var state = { connected: false, lastReplan: null, pendingDataset: null, scheduleStatus: null, pollTimer: null,
    currentDashboard: null, activeScheduleVersion: 'current', historyRequestToken: 0 };
  ...
  window.rememberCurrentSchedule = function (payload) {
    if (payload) state.currentDashboard = JSON.parse(JSON.stringify(payload));
  };
  window.receiveCurrentSchedule = function (payload) {
    window.rememberCurrentSchedule(payload);
    if (state.activeScheduleVersion === 'current') applyDashboard(payload);
  };
  window.loadScheduleHistory = function (month) {
    return jsonFetch('/api/schedule/history' + (month ? '?month=' + encodeURIComponent(month) : ''));
  };
  window.loadScheduleHistoryVersion = function (version) {
    return jsonFetch('/api/schedule/history/' + encodeURIComponent(version));
  };
  window.selectScheduleVersion = function (version) {
    var token = ++state.historyRequestToken;
    if (version === 'current') {
      state.activeScheduleVersion = 'current';
      if (state.currentDashboard) applyDashboard(state.currentDashboard);
      return Promise.resolve({ schedule_version: 'current', dashboard: state.currentDashboard });
    }
    return window.loadScheduleHistoryVersion(version).then(function (snapshot) {
      if (token !== state.historyRequestToken) return null;
      state.activeScheduleVersion = version;
      applyDashboard(snapshot.dashboard);
      return snapshot;
    });
  };
  function fetchDashboard(retries, silent) {
    fetch(API + '/api/dashboard').then(function (response) {
      ...
      window.receiveCurrentSchedule(data);
      fetchAnalysis();
  ...
  window.applyRushOrderCandidate = function (candidateVersion, optionId, confirmedBy, reason) {
    ...
    return jsonFetch('/api/replan/apply', {
      ...
      state.activeScheduleVersion = 'current';
      window.receiveCurrentSchedule(data.dashboard);
      window.loadScheduleHistory().then(function (history) {
        if (typeof window.setScheduleHistoryVersions === 'function') window.setScheduleHistoryVersions(history.versions || []);
      }).catch(function () {});
      fetchAnalysis();
  ...
  function bootstrap() {
    ensureUploadControls();
    fetchDashboard(60);
    window.loadScheduleHistory().then(function (data) {
      if (typeof window.setScheduleHistoryVersions === 'function') {
        window.setScheduleHistoryVersions(data.versions || []);
      }
    }).catch(function (error) { log('历史版本拉取失败：' + error.message); });
