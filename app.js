...
function registerCompletion(orderId, completedAt){
...
function archivePage(){
  ...
  const pendingRows = orders.slice(1).map(r=>`<div class="order-data"><span>${escapeHtml(r[0])}</span><span>${escapeHtml(r[1])}</span><span>${escapeHtml(r[2])}</span><span>${escapeHtml(r[3])}</span><span><button type="button" class="primary-button archive-go" data-complete="${escapeHtml(r[0])}">登记完工</button></span></div>`).join('')
  ...
  const archivedRows = archiveRecords.map(a=>`<div class="order-data">
      ...
      <span class="archive-meta">${escapeHtml(shortTime(a.archivedAt))} · 移出 ${(a.removedTasks||[]).length} 条任务 / ${(a.removedRisks||[]).length} 条风险</span>
...
function openArchiveForm(orderId){
  const host = $('#archiveForm');
  if(!host) return;
  const row = orders.find((r,i)=>i>0 && String(r[0])===orderId);
  host.hidden = false;
  host.innerHTML = `<div class="archive-form-inner">
      <b>登记完工：${escapeHtml(orderId)}</b>
      <span>计划交期 ${escapeHtml(row?row[3]:'—')}</span>
      <label>完工时间<input type="datetime-local" id="archiveCompletedAt" aria-label="完工时间" value="${toDatetimeLocal(new Date())}" /></label>
      <button type="button" class="primary-button" id="archiveConfirm">确认归档</button>
      <button type="button" id="archiveCancel">取消</button>
      <small>归档后该订单退出在排池：甘特任务移除、关联风险清除、KPI 按在排重算。可在下方「已归档」中撤回。</small>
    </div>`;
  ...
  $('#archiveConfirm').addEventListener('click',()=>{
    ...
    const rec = registerCompletion(orderId, at);
