...
function renderGantt(){
  ...
  if(canvas){const days=Number(canvas.style.getPropertyValue('--gantt-days'))||7;canvas.style.minWidth=`${116+126*days*Math.ceil(maxEnd/100)}px`;}
  $('#ganttRows').innerHTML=rows.map(row=>`<div class="gantt-row"><label><span>${escapeHtml(row.name)}</span><small>${escapeHtml(row.processes.join(' / ')||'换型')}</small></label><div class="timeline">${row.bars.map(({bar,task})=>{const code=barCode(bar,task);const title=String(bar.title||'').replace(/"/g,'&quot;').replace(/\n/g,' ⏎ ');return `<button data-task="${escapeHtml(code)}" data-task-order="${escapeHtml(bar.label||'')}" data-task-process="${escapeHtml(bar.process||'')}" class="task ${escapeHtml(bar.status||'')}" style="left:${Number(bar.left)||0}%;width:${Number(bar.width)||1}%" title="${escapeHtml(title)}">${escapeHtml(bar.label)}</button>`;}).join('')}</div></div>`).join('');
...
window.applyBackendData = function (d) {
  ...
  if (d.time_context) {
    ...
    if (timeNode) {
      ...
      timeNode.textContent = `北京时间 ${now || '—'} · 排产起点 ${baseline || '—'}`;
